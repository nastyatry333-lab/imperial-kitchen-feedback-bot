...
async def show_ticket_list(
    ...
    if list_type == "new":
        ...
        items = get_complaints(context, "closed")
    else:
...
async def panel_closed(update, context):
    query = update.callback_query
    if not is_admin(update):
        await query.answer("Нет доступа.", show_alert=True)
        return
    await query.answer()
    await show_ticket_list(query, context, "closed")


async def panel_search(update, context):
    query = update.callback_query
    if not is_admin(update):
        await query.answer("Нет доступа.", show_alert=True)
        return
    await query.answer()
    context.chat_data["admin_search"] = True
    await query.edit_message_text(
        "🔎 ПОИСК ОБРАЩЕНИЯ\n\n"
        "Отправьте следующим сообщением одно из:\n\n"
        "• IK-номер — например IK-0001\n"
        "• номер заказа\n"
        "• User ID клиента",
        reply_markup=back_admin(),
    )
...
def find_complaints(context, value):
    value = str(value).strip().lower()
    result = []
    for complaint in context.bot_data.get("complaints", {}).values():
        fields = (
            str(complaint.get("ticket", "")).strip().lower(),
            str(complaint.get("order", "")).strip().lower(),
            str(complaint.get("user_id", "")).strip().lower(),
        )
        if value in fields:
...
def main():
    ...
    application.add_handler(
        CallbackQueryHandler(panel_closed, pattern=r"^panel_closed$")
    )

    application.add_handler(
        CallbackQueryHandler(panel_search, pattern=r"^panel_search$")
    )
